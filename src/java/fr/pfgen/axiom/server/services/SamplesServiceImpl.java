...
public class SamplesServiceImpl extends RemoteServiceServlet implements SamplesService {
    ...
    @Override
    public int nbSamplesInFamily(String familyName) {
        String query = "SELECT * FROM ((samples s JOIN samples_in_families sf ON s.sample_id=sf.sample_id) JOIN families f on f.family_id=sf.family_id) WHERE family_name=\"" + familyName + "\"";
        return DatabaseUtils.countRowInQuery(pool, query, false);
    }
