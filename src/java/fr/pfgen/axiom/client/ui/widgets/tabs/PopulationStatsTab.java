...
public class PopulationStatsTab {
	...
	public PopulationStatsTab(final String populationName, final String tabID){
		...
		statsMenuItem.addClickHandler(new ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				if (hLayout.getMember(tabID+"_stats")!=null){
					...
					hLayout.addMember(constructProjectStats(populationName,tabID),1);
				}
			}
		});
		...
		userQcMenuItem.addClickHandler(new ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				if (hLayout.getMember(tabID+"_manageQC")!=null){
					...
					hLayout.addMember(constructQcManagementLayout(populationName, tabID));
				}
			}
		});
		...
		samplesQCMenuItem.addClickHandler(new ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				if (hLayout.getMember(tabID+"_QCgrid")!=null){
					...
					hLayout.addMember(construcQCListgrid(populationName,tabID),2);
				}
			}
		});
		...
		final IMenuButton statsButton = new IMenuButton(populationName);
		 ...
		 */
		MainArea.addTabToTopTabset("Stats : "+populationName,tabID, vlayout, true);
	}
	
	private HLayout constructQcManagementLayout(final String populationName, String tabID) {
		final HLayout hlayout = new HLayout(10);
		...
		IButton uploadButton = new IButton("Upload file", new com.smartgwt.client.widgets.events.ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				hlayout.addMember(constructUploadLayout(populationName));
			}
		});
		
		uploadButton.setAutoFit(true);
		uploadButton.setIcon("icons/Upload.png");
		uploadButton.setPrompt("Upload a tab delimited file containing sample informations for this population");
		...
		IButton downloadGraph = new IButton("Download graph", new com.smartgwt.client.widgets.events.ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				hlayout.addMember(constructDownloadQcLayout(populationName));
			}
		});
	...
	private VLayout constructDownloadQcLayout(final String populationName){
		...
		downloadGraph.addClickHandler(new com.smartgwt.client.widgets.events.ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				samplesQCService.makeQCGraphForPopulation(populationName, xAxis, yAxis, new AsyncCallback<String>() {
		...
		downloadTsv.addClickHandler(new com.smartgwt.client.widgets.events.ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				samplesQCService.makeQCTsvForPopulation(populationName, xAxis, yAxis, new AsyncCallback<String>() {
	...
	private VLayout constructUploadLayout(final String populationName){
		final VLayout layout = new VLayout(10);
		layout.setAutoHeight();
		layout.setAutoWidth();
		layout.setBorder("2px solid grey");
		...
		uploadStack.setUploadListener(new UploadListener() {
			...
			public void uploadComplete(final String filePath) {
				if (filePath.startsWith("Error")){
					...
					samplesQCService.checkUploadedQCForPopulation(populationName, filePath, new AsyncCallback<Map<String, String>>() {
						...
						public void onSuccess(Map<String, String> result) {
							...
							samplesQCService.addUserQCForPopulation(populationName, filePath, new AsyncCallback<String>() {
		...
		explanation.setContents("The&nbsp;uploaded&nbsp;file&nbsp;must&nbsp;contain&nbsp;a&nbsp;column&nbsp;named&nbsp;\"sampleName\"&nbsp;listing&nbsp;the&nbsp;names&nbsp;of&nbsp;the&nbsp;samples&nbsp;in&nbsp;the&nbsp;population."+
	...
	private VStack constructProjectStats(final String populationName, final String tabID){
		...
		//Listgrid for project details
		PopulationsListGrid populationListGrid = new PopulationsListGrid();
		populationListGrid.setDataSource(new PopulationsDS());
		Criteria criteria = new Criteria();
		criteria.addCriteria("population_name", populationName);
		populationListGrid.fetchData(criteria);
		
		//Label for plate number
		final Label plateNumber = new Label();
		plateNumber.setOverflow(Overflow.VISIBLE);
		plateNumber.setAutoHeight();
		plateNumber.setAutoWidth();
		platesService.getNumberOfPlatesInProject(populationName, new AsyncCallback<Integer>() {
			
			@Override
			public void onSuccess(Integer result) {
				plateNumber.setContents("Number&nbsp;of&nbsp;plates&nbsp;in&nbsp;population:&nbsp;"+result);
			}
			
			@Override
			public void onFailure(Throwable caught) {
				SC.warn("Can't retrieve number of plates in population "+populationName);
			}
		});
		...
		samplesService.nbSamplesInPopulation(populationName, new AsyncCallback<Integer>() {
			...
			@Override
			public void onSuccess(Integer result) {
				sampleNumber.setContents("Number&nbsp;of&nbsp;samples&nbsp;in&nbsp;population:&nbsp;"+result);
			}
			
			@Override
			public void onFailure(Throwable caught) {
				SC.warn("Can't retrieve number of samples in population "+populationName);
			}
		});
		...
		samplesService.nbSamplesInProjectWithoutQC(populationName, new AsyncCallback<Integer>() {
		
			@Override
			public void onSuccess(Integer result) {
				sampleWoQCNumber.setContents("Number&nbsp;of&nbsp;samples&nbsp;without&nbsp;QC&nbsp;in&nbsp;population:&nbsp;"+result);
			}
			
			@Override
			public void onFailure(Throwable caught) {
				SC.warn("Can't retrieve number of samples without QC in population "+populationName);
			}
		});
		
		vStack.addMember(stackTitle);
		vStack.addMember(populationListGrid);
		vStack.addMember(plateNumber);
	...
	private VStack construcQCListgrid(final String populationName, final String tabID){
		final VStack vStack = new VStack(10);
